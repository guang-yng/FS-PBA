...
--num_train_epochs 0 \
--output_dir "result/$TASK-$K-$HARD-$TRAIN_PARAM-$SEED-$LR-$MODEL-$TRIAL_IDTF" \
--seed $SEED \
--tag $TAG \
--template $TEMPLATE \
--mapping $MAPPING \
--training_params $TRAIN_PARAM \
$TASK_EXTRA \
$1
if (($? != 0)); then exit 1; fi
...
# rm -r result/$TASK-$K-$TRAIN_PARAM-$SEED-$MODEL-$TRIAL_IDTF/pytorch_model.bin
